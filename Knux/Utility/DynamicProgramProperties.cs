...
namespace Backend
{
    ...
    public class DynamicProgramProperties
    {
        ...
        public static IEnumerator Initialize(System.Action<bool> newInitializeCallback = null)
        {
            if (isInitialized)
            {
                Debug.Log("DynamicProperties| Already initialized");
                yield break;
            }
            ...
            Debug.Log("DynamicProperties| Request from GameSparks");
            new GameSparks.Api.Requests.GetPropertyRequest().SetPropertyShortCode("nuxDynamicProperties").Send((response) =>
            {
                bool propertiesLoaded = false;
                if (response.HasErrors)
                {
                    Debug.LogError("DynamicProperties| UNABLE TO LOAD DYNAMIC PROPERTIES -- " + response.Errors.JSON);
                    propertiesLoaded = false;
                }
                else
                {
                    Debug.Log("DynamicProperties| Received! -- LoadPropetiesForAllScopes");
                ...
                {
                    initializeCallback(propertiesLoaded);
                }
            });
        }
        ...
        public static void Initialize()
        {
            ...
            new GameSparks.Api.Requests.GetPropertyRequest().SetPropertyShortCode("nuxDynamicProperties").Send((response) =>
            {
                ...
                if (response.HasErrors)
                {
                    Debug.LogError("DynamicProperties| UNABLE TO LOAD DYNAMIC PROPERTIES");
                }
                else
                {
                    Debug.Log("DynamicProperties| Received! -- LoadPropetiesForAllScopes");
        ...
        public static bool TryGetValue<T>(string key, out T value)
        {
            ...
            if (properties.ContainsKey(key))
            {
                ...
                if (tmpObject != null)
                {
                    ...
                    {
                        Debug.LogError("DynamicProperties| Failed to convert value in " + key + ": to " + typeof(T).Name + "! Value is " + tmpObject.GetType().Name + "\n" + err.Message);
                    }
                }
            }
        ...
        public static void LoadPropertiesForAllScopes(string json)
        {
            if (json.Length > 0)
            {
                ...
                string releaseType = UnityEngine.Debug.isDebugBuild ? "Debug" : "Release";
            ...
            {
                Debug.LogError("DynamicProperties| No json retrieved for dynamic properties. You may not be connected to the internet!");
            }
        }
        ...
        private static void LoadPropertiesForScope(string json, string scopeKey)
        {
            if (json.Length > 0)
            {
                Debug.Log("DynamicProperties| Attempting to load dynamic properties for scope " + scopeKey);
                try
                {
                    ...
                    {
                        Debug.LogWarning("DynamicProperties| Dynamic properties not found for scope " + scopeKey);
                    }
                }
                catch (System.Exception e) // if someone makes a mistake in the dynamic properties, then we should try to go on without them
                {
                    ...
                    Debug.LogError("DynamicProperties| Error while parsing dynamic propertied! " + e.Message);
                }
            }
            else
            {
                Debug.LogError("DynamicProperties| Unable to load dynamic properties from empty json!");
            }
        }
        #endregion
    }
