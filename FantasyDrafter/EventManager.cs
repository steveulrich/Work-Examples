...
public class EventManager : MonoBehaviour
{
    ...
    public static EventManager instance
    {
        get
        {
            if (_appIsQuitting)
            {
                Logging.LogWarning("Someone is trying to use EventManager while it is being destroyed.");
                return null;
            }
            ...
            if (_instance == null)
                Logging.LogError("_instance is null and our code will almost never be expecting this to be the case.");
            return _instance;
        }
    }
    ...
    public bool AddListener(IEventListener listener, string eventName)
    {
        ...
        {
            Logging.Log("Event Manager: AddListener failed due to no listener or event name specified.");
            return false;
        }
        ...
        {
            Logging.Log("Event Manager: Listener: " + listener.GetType().ToString() + " is already in list for event: " + eventName);
    ...
    public bool TriggerEvent(IEvent evt)
    {
        ...
        if (!_listenerTable.ContainsKey(eventName))
        {
            Logging.LogWarning("Event Manager: Event \"" + eventName + "\" triggered has no listeners!");
        ...
        for (var i = listenerList.Count - 1; i >= 0; --i)
        {
            ...
            {
#if UNITY_EDITOR
                Logging.LogWarning("A listener for " + eventName + " is attached to a null game object and is being removed by the EventManger.");
#endif
                ((ArrayList)_listenerTable[eventName]).RemoveAt(i);
